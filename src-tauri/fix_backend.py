...
# Remove is_network_path import and uses
for file_path in glob.glob('src/**/*.rs', recursive=True):
    with open(file_path, 'r') as f:
        content = f.read()
    
    # Remove import
    content = re.sub(r'use crate::mounts::is_network_path;\n?', '', content)
    
    # Replace usages of `is_network_path(...)` with `false`
    content = re.sub(r'is_network_path\([^)]+\)', 'false', content)
