<script lang="ts">
  ...
  function toTreeNode(node: any): TreeViewNode {
    ...
    return {
      ...
      isActive: (appState.panes?.[appState.activePane === 'secondary' ? 'secondary' : 'primary']?.path || '') === path,
      isExpanded,
  ...
  let treeRoots = $derived.by(() => {
    return (appState.drives || []).map((drive: DriveInfo) => ({
      ...
      isActive: (appState.panes?.[appState.activePane === 'secondary' ? 'secondary' : 'primary']?.path || '') === drive.path,
